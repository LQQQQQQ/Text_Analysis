...
def fetch_collobert_and_weston(normalize=True, lower=False, clean_words=False):
    """
    Fetches Collobert & Weston embeddings.
    ...
    References
    ----------
    Project website: https://ronan.collobert.com/senna/
    ...
    download_file = {
            "wiki-2007": "https://nextcloud.teklia.com/index.php/s/jMzC8JMc429aeAM/download"}
    ...
    if not path.exists(path.join(_get_dataset_dir("embeddings"), embedding_file["wiki-2007"][50])):
        _fetch_file(url=download_file["wiki-2007"],
                           data_dir="embeddings",
                           uncompress=False,
                           verbose=1)
        os.rename(path.join(_get_dataset_dir("embeddings"), 'download'), 
                  path.join(_get_dataset_dir("embeddings"), embedding_file["wiki-2007"][50]))
    
    return load_embedding(path.join(_get_dataset_dir("embeddings"), embedding_file["wiki-2007"][50]),
                           format="glove",
                           normalize=normalize,
